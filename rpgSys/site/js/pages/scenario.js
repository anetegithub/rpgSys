...
function renderCharacteristics() {
    ...
    for (var i = 0; i < text.length; i++) {
        if (text[i] == "<hr/>") {
            ...
            html += "<div class='form-group'><label class='col-sm-3 control-label' for='" + ids[i] + "Text' id='" + ids[i] + "Labbel'>" + text[i] + " </label><div class='col-sm-6'><span class='picker' selectr='" + ids[i] + "'></span></div></div>";
        }
    }
...
$('#addReward').click(function () {
    if ($('#rewardName').val() != "") {
        if ($('#rewardName').val().length <= 100) {
            ...
            var ifs = "";
            if (type != "Всем") {
                ifs = $('#rewardCharAdditionalTEXT').val();
            }
            ...
            html += "<input type='hidden' value='" + ifs + "' />";
...
$('#rewardRare').change(function () {
    ...
    switch (str) {
        ...
        case "Редкий ": $('#rewardRare').addClass('btn-warning'); break;
        case "Легендарный ": $('#rewardRare').addClass('btn-success'); break;
        case "Артефакт ": $('#rewardRare').addClass('btn-danger'); break;
    }
}).change();
...
function gatherData() {
    ...
    for (var i = 0; i < $('#characterList').children().length; i++) {
        ...
        var CurrentNpcStats = NpcStats(stats);
        data['Npcs'].push({ "Name": $('#characterList').children()[i].children[1].innerText, "View": $('#characterList').children()[i].children[4].value, "Specification": $('#characterList').children()[i].children[5].value, "NpcStats": CurrentNpcStats });
    }
    ...
    for (var i = 0; i < $('#rewardPROTECTList').children().length; i++) {
        var stats = [];
        for (var j = 1; j < $('#rewardPROTECTList').children()[i].children[7].children[1].children[0].children.length; j += 2) {
            var n = $('#rewardPROTECTList').children()[i].children[7].children[1].children[0].children[j].value.split(':')[0];
            var v = $('#rewardPROTECTList').children()[i].children[7].children[1].children[0].children[j].value.split(':')[1];
            stats.push(RewardStat(n, v));
        }
        data['Rewards'].push({
            "Name": $('#rewardPROTECTList').children()[i].children[2].innerText,
            "Target": RuneString($('#rewardPROTECTList').children()[i].children[1].innerText),
            "Rare": RuneString($('#rewardPROTECTList').children()[i].children[5].value),
            "Conditions": $('#rewardPROTECTList').children()[i].children[6].value,
            "Additional": $('#rewardPROTECTList').children()[i].children[4].value,
            "RewardStats": stats
        });
    }
...
function NpcStats(stats) {
    ...
    o.DEF = stats[0].Value;
    o.DLS = stats[0].Value;
    o.nDEF = stats[0].Value;
    o.mDEF = stats[0].Value;
    o.ATK = stats[0].Value;
    o.MID = stats[0].Value;
    o.MAD = stats[0].Value;
    o.WID = stats[0].Value;
...
function InitRewardDropDowns() {
    $.getJSON('../api/scenario/enums').done(function (data) {
        var html = "";
        for (var i = 0; i < data[0].length; i++) {
            html += "<option class='form-control'>" + data[0][i].Value + "</option>";
        };
        $('#rewardChar').html(html);

        html = "";
        for (var i = 0; i < data[1].length; i++) {
            html += "<option class='form-control'>" + data[1][i].Value + "</option>";
        };
        $('#rewardCharact').html(html);

        var html = "";
        var colors = new Array("btn-default", "btn-info", "btn-warning", "btn-success", "btn-danger");
        var j=0;
        for (var i = 0; i < data[2].length; i++) {
            html += "<option class='form-control " + colors[j] + "'>" + data[2][i].Value + "</option>";
            j++;
            if (j == 5)
                j = 0;
        };
        $('#rewardRare').html(html);
    });
};
...
function RewardStat(Name, Value) {
    o = new Object();
    o.RewardInfo = RuneString(Name);
    o.Value = Value;
    return o;
};